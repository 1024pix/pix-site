...
export default function useMainNavigationLinks(
  ...
  return navigationZoneItems.reduce((formattedData: Array<FormattedMenuItem>, currentItem) => {
    const isCategory = currentItem.categoryName.length > 0 && currentItem.categoryName[0].text;

    if (!isCategory) {
      formattedData.push({
        name: _getTextFromArray(currentItem.menuName),
        beforeIcon: currentItem.beforeIcon,
        alternativeTextForBeforeIcon: _getTextFromArray(currentItem.alternativeTextForBeforeIcon),
        afterIcon: currentItem.afterIcon,
        alternativeTextForAfterIcon: _getTextFromArray(currentItem.alternativeTextForAfterIcon),
    ...
    if (existingSectionIndex !== -1) {
      ...
      currentCategory.sections.push(
        _createSection(currentTitle, _getTextFromArray(currentItem.menuName), currentItem.menuLink.url),
      );
    }
    return formattedData;
  }, []);
...
const _createSection = (menuSectionTitle: string, menuName: string, menuLink: string): Section => {
  return {
