...
export function documentFetcher(prismic, i18n = { defaultLocale: 'fr-fr' }) {
  ...
  return {
    ...
    findHotNewsBanner: async () => {
      const documents = await prismic.api.query(prismic.predicates.at('document.type', DOCUMENTS.HOT_NEWS), {
        lang: locale,
      });
    ...
    findMainNavigation: async () => {
      const documents = await prismic.api.query(
        [
          ...
          prismic.predicates.at(`my.${DOCUMENTS.MAIN_NAVIGATION}.navigation_for`, process.env.SITE),
        ],
        { lang: locale },
      );
      return documents.results[0];
    ...
    findMainFooter: async () => {
      const documents = await prismic.api.query(
        [
          ...
          prismic.predicates.at(`my.${DOCUMENTS.MAIN_FOOTER}.footer_for`, process.env.SITE),
        ],
        { lang: locale },
      );
      return documents.results[0];
    ...
    findNewsItems: async ({ page, pageSize } = { page: 1, pageSize: 20 }) => {
      const documents = await prismic.api.query(prismic.predicates.at('document.type', DOCUMENTS.NEWS_ITEM), {
        page,
        pageSize,
    ...
    getNewsItemByUid: async (slug) => {
      const document = await prismic.api.query(prismic.predicates.at(`my.${DOCUMENTS.NEWS_ITEM}.uid`, slug), {
        lang: locale,
      });
  ...
  async function getSimplePageByUid(uid) {
    const document = await prismic.api.query(prismic.predicates.at(`my.${DOCUMENTS.SIMPLE_PAGE}.uid`, uid), {
      lang: locale,
    });
    return document.results[0];
  }
  ...
  async function getFormPageByUid(uid) {
    const document = await prismic.api.query(prismic.predicates.at(`my.${DOCUMENTS.FORM_PAGE}.uid`, uid), {
      lang: locale,
    });
    return document.results[0];
  }
  ...
  async function getSlicesPageByUid(uid) {
    const document = await prismic.api.query(prismic.predicates.at(`my.${DOCUMENTS.SLICES_PAGE}.uid`, uid), {
      lang: locale,
    });
    return document.results[0];
  }
